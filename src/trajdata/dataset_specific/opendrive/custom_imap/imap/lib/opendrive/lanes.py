...
from imap.lib.common import Point3d, binary_search, shift_t
from imap.lib.draw import draw_line
from imap.lib.opendrive.common import convert_speed
from imap.lib.opendrive.plan_view import Geometry
from imap.lib.opendrive.profile import ElevationProfile
...
def is_adjacent(road_marks) -> bool:
    ...
    if (
        road_mark.roadmark_type == "botts dots"
        or road_mark.roadmark_type == "broken broken"
        or road_mark.roadmark_type == "broken solid"
        or road_mark.roadmark_type == "broken"
        or road_mark.roadmark_type == "none"
    ):
        return True
    return False
...
class Lane:
    def __init__(self, lane_id=None, lane_type=None, level=None, direction=None):
    ...
    def generate_boundary(self, left_boundary, start_s):
        ...
        for point3d in left_boundary:
            ...
            rpoint3d = shift_t(point3d, -self.direction * width)
            self.right_boundary.append(rpoint3d)

            cpoint3d = shift_t(point3d, -self.direction * width / 2)
...
class LaneSection:
    ...
    def get_lane(self, lane_id: str):
        ...
        if lane_id_int < 0:
            return self.right[abs(lane_id_int) - 1]
        else:
            return self.left[len(self.left) - lane_id_int]
    ...
    def get_cross_section(self, direction):
        if direction == "start":
            if self.left and self.right:
                ...
                return [
                    leftmost_lane.right_boundary[-1],
                    rightmost_lane.right_boundary[0],
                ]
            elif self.left:
                leftmost_lane, rightmost_lane = self.left[0], self.left[-1]
                return [
                    leftmost_lane.right_boundary[-1],
                    rightmost_lane.left_boundary[-1],
                ]
            elif self.right:
                leftmost_lane, rightmost_lane = self.right[0], self.right[-1]
                return [
                    leftmost_lane.left_boundary[0],
                    rightmost_lane.right_boundary[0],
                ]
            else:
                return []
            ...
            if self.left and self.right:
                leftmost_lane, rightmost_lane = self.left[0], self.right[-1]
                return [
                    leftmost_lane.right_boundary[0],
                    rightmost_lane.right_boundary[-1],
                ]
            elif self.left:
                leftmost_lane, rightmost_lane = self.left[0], self.left[-1]
                return [
                    leftmost_lane.right_boundary[0],
                    rightmost_lane.left_boundary[0],
                ]
            elif self.right:
                leftmost_lane, rightmost_lane = self.right[0], self.right[-1]
                return [
                    leftmost_lane.left_boundary[-1],
                    rightmost_lane.right_boundary[-1],
                ]
            else:
                return []
...
class Lanes:
    def __init__(self):
        self.lane_offsets = []
        self.lane_sections: List[LaneSection] = []
    ...
    def process_lane_sections(
        self, reference_line: IntervalTree, elevation_profile: ElevationProfile
    ):
        sampling_dist = global_var.get_element_value("sampling_length")
        for lane_section in self.lane_sections:
            start_s = lane_section.s
            end_s = lane_section.end_s
            reference_line_in_section = []

            for geom_interval in sorted(reference_line[start_s:end_s]):
                geom: Geometry = geom_interval.data
                left_s, right_s = max(start_s, geom.s), min(end_s, geom.s + geom.length)
                ...
                if type(geom) is Geometry:
                    ...
                    reference_line_in_section.extend(
                        geom.arange(left_s, right_s, sampling_dist, elevation_profile)
                    )

            # Processing offsets.
            if self.have_offset():
                ref_point: Point3d
                for ref_point in reference_line_in_section:
                    offset = self.get_offset_by_s(ref_point.s)
                    ref_point.shift_t(offset)
