    ...
    def post_process(self):
        ...
        for road_id, road in self.roads.items():
            ...
            if road.link.predecessor.element_type == "junction":
                road.link.predecessor_junction = self.junctions[
                    road.link.predecessor.element_id
                ]
            elif road.link.predecessor.element_type == "road":
                road.link.predecessor_road = self.roads[
                    road.link.predecessor.element_id
                ]

            if road.link.successor.element_type == "junction":
                road.link.successor_junction = self.junctions[
            ...
            if road.junction_id != "-1":
                ...
                if not self.junctions[road.junction_id].is_incoming_road(
                    ...
                    if (
                        successor_id
                        not in self.junctions[road.junction_id].predecessor_dict
                    ):
                        self.junctions[road.junction_id].predecessor_dict[
                            successor_id
                        ] = []
                    self.junctions[road.junction_id].predecessor_dict[
                        successor_id
