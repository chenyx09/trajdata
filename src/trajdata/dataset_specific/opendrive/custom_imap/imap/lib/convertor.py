...
from imap.lib.convex_hull import aabb_box, convex_hull
from imap.lib.draw import draw_line, show
from imap.lib.opendrive.map import Map
from imap.lib.proto_utils import write_pb_to_bin_file, write_pb_to_text_file
from modules.map.proto import map_lane_pb2, map_pb2, map_road_pb2
...
def to_pb_boundary_type(opendrive_boundary_type):
    if (
        opendrive_boundary_type.boundary_type is None
        or opendrive_boundary_type.color is None
    ):
...
class Opendrive2Apollo(Convertor):
    ...
    def convert_header(self):
        ...
        if proj is not None:
            ...
            zone_id = 0
            self.pb_map.header.projection.proj = (
    ...
    def add_basic_info(self, pb_lane, xodr_road, idx, lane):
        pb_lane.id.id = "road_{}_lane_{}_{}".format(
            xodr_road.road_id, idx, lane.lane_id
        )
        ...
        for predecessor_road, dirct in predecessors:
            ...
            if dirct == "predecessor":
                for predecessor_lane in predecessor_road.lanes.lane_sections[0].left:
                    if (
                        predecessor_lane.link.predecessor
                        and predecessor_lane.link.predecessor.link_id == lane.lane_id
                    ):
                ...
                for predecessor_lane in predecessor_road.lanes.lane_sections[
                    section_id
                ].right:
                    if (
                        predecessor_lane.link.successor
                        and predecessor_lane.link.successor.link_id == lane.lane_id
                    ):
                        pb_lane.predecessor_id.add().id = "road_{}_lane_{}_{}".format(
                            predecessor_road.road_id,
                            section_id,
                            predecessor_lane.lane_id,
                        )
            else:
        ...
        if idx == 0:
            ...
            if predecessor_junction is not None:
                ...
                for connection in predecessor_junction.connections:
                    lane_link = connection.incoming_lane_link(
                        xodr_road.road_id, lane.lane_id
                    )
        ...
        if idx == 0:
            ...
            if predecessor_road_id and lane.link.predecessor:
                ...
                if xodr_road.link.predecessor.contact_point == "start":
                    ...
                    section_id = (
                        len(xodr_road.link.predecessor_road.lanes.lane_sections) - 1
                    )
        ...
        if idx == cur_n - 1:
            ...
            if successor_road_id and lane.link.successor:
                ...
                if xodr_road.link.successor.contact_point == "start":
                    ...
                    section_id = (
                        len(xodr_road.link.successor_road.lanes.lane_sections) - 1
                    )
    ...
    def add_road_section_boundary(self, pb_road_section, lane_section):
        ...
        self.add_road_section_curve(
            left_boundary_edge, leftmost_boundary, leftmost_length
        )
        self.add_road_section_curve(
            right_boundary_edge, rightmost_boundary, rightmost_length
        )
    ...
    def convert_roads(self):
        for _, xodr_road in self.xodr_map.roads.items():
            ...
            draw_line(
                ...
                "r",
                reference_line=True,
                label="reference line " + str(pb_road.id.id),
            )
    ...
    def convert(self):
        ...
        if self.output_file_name is None:
            show(
                need_save=global_var.get_element_value("need_save_figure"),
