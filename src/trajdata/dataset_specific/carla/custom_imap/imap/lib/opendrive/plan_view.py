...
class Spiral(Geometry):
  ...
  def get(self, local_s: float, local_t: float, local_h: float, elevation_profile) -> Point3d:
  ...
  def sampling(self, delta_s, elevation_profile):
    ...
    for i in range(sample_count):
      ...
      point3d = self.get(local_s, 0, 0, elevation_profile)
      points.append(point3d)
    return points
...
class Poly3(Geometry):
  ...
  def get(self, local_s: float, local_t: float, local_h: float, elevation_profile) -> Point3d:
    cubic_s, cubic_t, theta = cubic_polynoms(self.a, self.b, self.c, self.d, local_s)
...
class PlanView:
  ...
  def get_xyz_at_sthe(self, s: float, t: float, h: float, elevation_profile) -> Point3d:
    idx = binary_search([g.s for g in self.geometrys], s)
    return self.geometrys[idx].get(s - self.geometrys[idx].s, t, h, elevation_profile)
