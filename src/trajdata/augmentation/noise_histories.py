...
class NoiseHistories(BatchAugmentation):
    ...
    def apply_agent(self, agent_batch: AgentBatch) -> None:
        ...
        if agent_batch.history_pad_dir == PadDirection.BEFORE:
            ...
            len_mask = ~mask_up_to(agent_batch.agent_hist_len, delta=-1,
                                   max_len=agent_batch.agent_hist.shape[1]).unsqueeze(-1)
            agent_hist_noise[len_mask.expand(-1, -1, agent_hist_noise.shape[-1])] = 0

            len_mask = ~mask_up_to(agent_batch.neigh_hist_len, delta=-1,
                                   max_len=agent_batch.neigh_hist.shape[2]).unsqueeze(-1)
            neigh_hist_noise[
