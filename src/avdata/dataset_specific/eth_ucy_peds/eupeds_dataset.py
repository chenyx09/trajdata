...
from typing import Dict, List, Optional, Set, Tuple, Type, Union, Final
...
from avdata.dataset_specific.scene_records import EUPedsRecord
...
class EUPedsDataset(RawDataset):
    def compute_metadata(self, env_name: str, data_dir: str) -> EnvMetadata:
        scene_splits: Dict[str, List[str]] = {
            ...
            "train_loo": [sn + "_train" for sn in TRAIN_SCENES if sn not in TEST_SCENES[env_name]],
            "val_loo": [sn + "_val" for sn in TRAIN_SCENES if sn not in TEST_SCENES[env_name]],
        ...
        dataset_parts_loo: List[str] = [
            "train_loo", "val_loo", "test_loo"
        ]
        ...
        env_metadata.scene_tags += [SceneTag((env_name, loo_part, )) for loo_part in dataset_parts_loo]
        return env_metadata
    ...
    def load_dataset_obj(self, verbose: bool = False) -> None:
        ...
        for scene_name in TRAIN_SCENES:
            ...
            self.dataset_obj[scene_name + "_train"] = data[data["frame_id"] < TRAINVAL_FRAME_SPLITS[scene_name]]
            
            # Creating a copy because we have to fix the frame_id values (to ensure they start from 0).
            self.dataset_obj[scene_name + "_val"] = data[data["frame_id"] >= TRAINVAL_FRAME_SPLITS[scene_name]].copy()
            self.dataset_obj[scene_name + "_val"]["frame_id"] -= TRAINVAL_FRAME_SPLITS[scene_name]
    ...
    def _get_matching_scenes_from_obj(
        ...
        for idx, (scene_name, scene_df) in enumerate(self.dataset_obj.items()):
            ...
            if "eth" in scene_name or "hotel" in scene_name:
                scene_location = "zurich"
            else:
                scene_location = "cyprus"
            ...
            if (scene_location in scene_tag or "loo" in scene_split) and scene_split in scene_tag and scene_desc_contains is None:
    ...
    def _get_matching_scenes_from_cache(
        ...
        all_scenes_list: List[EUPedsRecord] = env_cache.load_env_scenes_list(
            self.name
        )
