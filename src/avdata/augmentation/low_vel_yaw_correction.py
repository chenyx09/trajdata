...
class LowSpeedYawCorrection(Augmentation):
    def __init__(self, speed_threshold: float = 0.) -> None:
    ...
    def apply(self, scene_data_df: pd.DataFrame) -> None:
        ...
        agent_ids: pd.Series = scene_data_df.index.get_level_values(0).astype("category")
        ...
        border_mask: np.ndarray = np.concatenate([[0], np.nonzero(agent_ids[1:] != agent_ids[:-1])[0] + 1])
        ...
        scene_data_df.loc[:, "heading"] = scene_data_df.groupby("agent_id")["yaw_diffs"].cumsum()
